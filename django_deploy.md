...
note if you renamed the key then you will get permission denied

so do
$ ssh-add /path for key  eg:ssh-add ./.ssh/id_rsa_do

if you get Could not open a connection to your authentication agent.
do this first open the agent
$ eval $(ssh-agent)
```
...
### Exit and relogin with createduser

````
# exit
$ ssh djangoadmin@139.59.15.101

ypu will get 
djangoadmin@139.59.15.101: Permission denied (publickey).
...
# mkdir .ssh
# cd .ssh
# nano authorized_keys  instead you can use vim as well
...
From your local machine, create a requirements_for_server.txt with your app dependencies. Make sure you push this to your repo

```
$ pip freeze > requirements_for_server.txt

comment that is not required for server and only neaded for vscode
```
